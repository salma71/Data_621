...
set.seed(42)
library(caret)
preProcValues <- preProcess(data_train %>% 
                          dplyr::select(TEAM_BATTING_SO, TEAM_BASERUN_SB, TEAM_BASERUN_CS, TEAM_PITCHING_SO, TEAM_FIELDING_DP),
                            method = c("knnImpute"),
                            k = 20,
                            knnSummary = mean)
```
...
```{r}
mi_m1 <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_FIELDING_E, data = mod_data)
summary(mi_m1)
```
```{r}
plot(mi_m1)
