...
We find that observation 457 is influential outliers as they have the largest cook distance and their standardized residuals are more 3 SD's away from the mean
...
```{r}
augment(ilp) %>% mutate(index = 1:n()) %>% ggplot( aes(index, .std.resid)) + geom_point(aes(color=target)) + labs(y= "Standardized Residuals", x=" ")
```
...
```{r}
#the two influencial points, observations and 457
