...
The model that used additional transformed features follow the same observations as above, residual deviance increases and AIC decreases with smaller models. While the residual deviance is smaller than in the base model subsets, we have gained 1.9% in accuracy.

```{r}
trans_models.df
```

As outlined above, model mt5 yields the best prediction accuracy on the testing data.
