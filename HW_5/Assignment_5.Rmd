...
```{r}
# poisson model with the imputed values
model2a <- glm(TARGET ~ ., family = quasipoisson(link='log'), data_train_imputed)
summary(model2a)
print('Goodness of Fit Test:')
with(model2a, cbind(res.deviance = deviance, df = df.residual, p = pchisq(deviance, df.residual, lower.tail=FALSE)))
```
...
```{r}
model3 <- zeroinfl(TARGET ~., data_train_imputed, dist = 'poisson')
summary(model3)
```

```{r}
model3b <- zeroinfl(TARGET ~ . - FixedAcidity - Density, data_train_imputed, dist = 'poisson')
summary(model3b)
```
...
```{r}
table(round(model2$fitted.values))
table(round(model3$fitted.values))
table(round(model3b$fitted.values))
```
...
pred_train <- data.frame(TARGET=data_train_imputed$TARGET, 
                         ...
                         model3b=model3b$fitted.values)
pred_train <- round(pred_train, 2)
pred_train
```
...
model3b_fitted.values <- factor(round(model3b$fitted.values),levels=rev(0:9))

confusionMatrix(model2_fitted.values, factor(data_train_imputed$TARGET,levels=rev(0:9)))
confusionMatrix(model3_fitted.values, factor(data_train_imputed$TARGET,levels=rev(0:9)))
confusionMatrix(model3b_fitted.values, factor(data_train_imputed$TARGET,levels=rev(0:9)))
```
