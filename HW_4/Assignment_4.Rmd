...
```{r}
mod2_data <- data_train %>% select(-c('TARGET_AMT','INDEX'))
#mod2_data <- data_train_balanced %>% select(-c('TARGET_AMT','INDEX'))
...
```{r}
m2a <- score_model('model.full', model.full, mod2_data, output = TRUE)
m2a$df_info
models.df <- rbind(models.df,m2a$df_info)
```

The summary output of the reduced model retains a number of statistically significant predictors. 

```{r}
m2b <- score_model('model.reduced', model.reduced, mod2_data, output = TRUE)
m2b$df_info
models.df <- rbind(models.df,m2b$df_info)
```
...
```{r}
mod2_data_bal <- data_train_balanced %>% select(-c('TARGET_AMT','INDEX'))
...
```{r}
m2c <- score_model('modelbal.full', modelbal.full, mod2_data_bal, output = FALSE)
m2c$df_info
models.df <- rbind(models.df,m2c$df_info)

m2d <- score_model('modelbal.reduced', modelbal.reduced, mod2_data_bal, output = FALSE)
m2d$df_info
models.df <- rbind(models.df,m2d$df_info)
```
...
mod3_data <- data_train %>% select(-c('TARGET_AMT','INDEX'))

# Split the data into training and test set
set.seed(42)
training.samples <- mod3_data$TARGET_FLAG %>% createDataPartition(p = 0.8, list = FALSE)
train.data <- mod3_data[training.samples, ]
test.data <- mod3_data[-training.samples, ]
...
```{r}
data_train2 <- data_train %>% select(c(-'INDEX')) %>% filter(TARGET_FLAG==1) %>% select(-c('TARGET_FLAG'))
```

```{r}
lm5 <- lm(TARGET_AMT ~ ., data_train2)
summary(lm5)
```
...
# Split the data into training and test set
set.seed(42)
training.samples <- data_train2$TARGET_AMT %>% createDataPartition(p = 0.8, list = FALSE)
train.data <- data_train2[training.samples, ]
test.data <- data_train2[-training.samples, ]

# Dummy code categorical predictor variables
x <- model.matrix(TARGET_AMT~., train.data)[,-1]
# Convert the outcome (class) to a numerical variable
y <- train.data$TARGET_AMT
...
set.seed(42)
# Find the best lambda using cross-validation
cv.lasso <- cv.glmnet(x, y, alpha = 0.1)
plot(cv.lasso)
```

```{r}
paste0('lambda.min = ',cv.lasso$lambda.min)
paste0('lambda.1se = ',cv.lasso$lambda.1se) # gives the simplest model but also lies within one standard error of the optimal value of lambda
```
...
```{r}
# Final model with lambda.min
lasso.model.min <- glmnet(x, y, alpha = 0.1,
                      lambda = cv.lasso$lambda.min)
# Make prediction on test data
x.test <- model.matrix(TARGET_AMT ~., test.data)[,-1]
probabilities <- lasso.model.min %>% predict(newx = x.test)
predicted.classes <- ifelse(probabilities > 0.5, 1, 0)
# Model accuracy
observed.classes <- test.data$TARGET_AMT
mean(predicted.classes == observed.classes)
```

```{r}
# Final model with lambda.1se
lasso.model.se1 <- glmnet(x, y, alpha = 1,
                      lambda = cv.lasso$lambda.1se)
# Make prediction on test data
x.test <- model.matrix(TARGET_AMT ~., test.data)[,-1]
probabilities <- lasso.model.se1 %>% predict(newx = x.test)
